...
// Lumen to never linking TLS for the SERVER side ("TLS... belongs to the
...
// there is no reverse proxy sitting between Lumen and a third-party HTTPS
...
Value do_request(const std::string& method, const std::string& url,
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "Lumen/1.0");
    // Secure by default, and never made configurable to "off" from Lumen
