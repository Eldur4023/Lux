...
//   });
//
//   lux::log().info("cache warmed, ", n, " entries");
//   lux::log().error("payment provider unreachable: ", err);
//
...
struct LoggerOptions {
    ...
    // Rotate when the current file reaches this size: lux.log is renamed
    // to lux.1.log (then .2, .3, …) and a fresh lux.log is started.
...
private:
    ...
    void open_file_locked() {
        ...
        if (!file_) {
            std::cerr << "[lux] logger: cannot open "
